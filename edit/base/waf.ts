...
type ZvoogZone = {
	midi: number//0 //MIDI program
	, originalPitch: number// 84*100-36*100 //root pitch in cent, may be transpose up/down
	, keyRangeLow: number// 12*3+6 //zone low key
	, keyRangeHigh: number// 127 //zone high key
	, loopStart: number// 0 //loop tart in seconds
	, loopEnd: number// 0 //loop end in seconds
	, coarseTune: number// 0 //use fine tune
	, fineTune: number// -11 //tune correction in cents
	, sampleRate: number// 48000 //file sample rate
	//, ahdsr?: any// true // see example
	, ahdsrPoints: ZvoogAHDSR[]// true // see example
	//, file?: string//'//uUxAAAAAAAAAAAAAAAAAAAAAAAWGluZwAAAA8AAADzAA...
	, buffer: AudioBuffer
	, delay: number
	// ,sample?:string
};
...
class ZvoogPlayer {
	...
	waitGlobalVariable(variableName: string, onFinish: () => void) {
	...
	zones(variableName: string): ZvoogPreset | null {
		if (window[variableName]) {
			let preset: any = window[variableName] as any;
			for (let i = 0; i < preset.zones.length; i++) {
				if (!(preset.zones[i].buffer)) {
					return null;
				}
			}
			return window[variableName] as ZvoogPreset;
		} else {
			return null;
		}
	}
	afterAdjust(variableName: string, onFinish: () => void) {
		if (window[variableName]) {
			let preset: any = window[variableName] as any;
			let done = true;
			for (let i = 0; i < preset.zones.length; i++) {
	...
	adjustAHDSR(zone: any) {
		if (zone.ahdsr) {
			if (zone.ahdsr.length > 0) {
				zone.ahdsrPoints = [];
				for (let i = 0; i < zone.ahdsr.length; i++) {
	...
	adjustZone(audioContext: AudioContext, zone: any) {
		if (zone.buffer) {
			...
			if (zone.sample) {
				let decoded: string = atob(zone.sample);
				zone.buffer = audioContext.createBuffer(1, decoded.length / 2, zone.sampleRate);
				let float32Array: Float32Array = zone.buffer.getChannelData(0);
				let b1: number;
				let b2: number;
				let n: number;
				for (let i: number = 0; i < decoded.length / 2; i++) {
				...
				if (zone.file) {
					let datalen: number = zone.file.length;
					let arraybuffer: ArrayBuffer = new ArrayBuffer(datalen);
					let view: Uint8Array = new Uint8Array(arraybuffer);
					let decoded: string = atob(zone.file);
					let b: number;
	...
	numValue(aValue: string, defValue: number): number {
	...
	findZone(preset: ZvoogPreset, pitch: number): ZvoogZone | null {
		for (let i: number = preset.zones.length - 1; i >= 0; i--) {
			let zone: ZvoogZone = preset.zones[i];
	...
	findEnvelope(audioContext: AudioContext, target: AudioNode, preset: ZvoogPreset): ZvoogEnvelope {
		let envelope: ZvoogEnvelope | null = null;
		...
		if (!(envelope)) {
			envelope = {
				...
				, cancel: function (): void {
					if (envelope) if (envelope.when + envelope.duration > audioContext.currentTime) {
	...
	setupEnvelope(audioContext: AudioContext, envelope: ZvoogEnvelope, zone: ZvoogZone//, volume: number
		, when: number, waveDuration: number, noteDuration: number): void {
		envelope.base.gain.setValueAtTime(this.noZeroVolume(0), audioContext.currentTime);
		let playDuration: number = noteDuration;
		let envPoints: ZvoogAHDSR[] = zone.ahdsrPoints;
		...
		let currentVolume: number = envPoints[0].volume;
		envelope.base.gain.setValueAtTime(this.noZeroVolume(currentVolume * this.wafratio), when);
		for (let i: number = 0; i < envPoints.length; i++) {
			if (currentDuration + envPoints[i].duration > playDuration) {
				let remains = envPoints[i].duration + currentDuration - playDuration
				let ratio = 1 - remains / envPoints[i].duration;
				let revolume = currentVolume - ratio * (currentVolume - envPoints[i].volume);
	...
	calculateDuration(tempo:number,pitches: ZvoogPitch[]): number {
		let r: number = 0;
		for (let i = 0; i < pitches.length; i++) {
			//if(pitches.length>1)console.log(r,pitches[i].duration);
			if (pitches[i].duration ) r = r + duration2time(tempo, pitches[i].duration);

		}
		//if(pitches.length>1)console.log(r,pitches);
		return r;
	}
	queueWaveTable(audioContext: AudioContext, target: AudioNode, preset: ZvoogPreset, when: number
		,tempo:number, pitches: ZvoogPitch[]): ZvoogEnvelope | null {
		if (audioContext.state == 'suspended') {
			console.log('audioContext.resume');
			audioContext.resume();
		}
		let pitch: number = pitches[0].pitch;
		let zone: ZvoogZone | null = this.findZone(preset, pitch);
		if (!zone) {
		...
		let duration: number = this.calculateDuration(tempo,pitches);
		if (when > 0 && when + duration < audioContext.currentTime) {
			console.log('skip');
			return null;
		}
		let baseDetune: number = zone.originalPitch - 100.0 * zone.coarseTune - zone.fineTune;
		let currentPlaybackRate: number = 1.0 * Math.pow(2, (100.0 * pitch - baseDetune) / 1200.0);
		let startWhen: number = when;
		if (startWhen < audioContext.currentTime) {
			startWhen = audioContext.currentTime;
		}
		let waveDuration: number = duration + this.afterTime;
		...
		let envelope: ZvoogEnvelope = this.findEnvelope(audioContext, target, preset);
		...
		if (pitches.length > 1) {
			...
			let newWhen: number = startWhen;
			for (let i: number = 1; i < pitches.length; i++) {
				let newPlaybackRate: number = 1.0 * Math.pow(2, (100.0 * pitches[i].pitch - baseDetune) / 1200.0);
	...
	instrumentInfo(n: number): { variable: string, url: string, title: string } {
		var key = this.instrumentKeys()[n];
		let num = key.substring(0, 3);
		var p = parseInt(num);
		return {
			variable: '_tone_' + key,
	...
	drumInfo(n: number): { variable: string, url: string, title: string, pitch: number } {
		var key = this.drumKeys()[n];
		let num = key.substring(0, 2);
		var p = parseInt(num);
		return {
			variable: '_drum_' + key,
	...
	findInstrument(program: number): number {
		for (var i = 0; i < this.instrumentKeys().length; i++) {
			let num = this.instrumentKeys()[i].substring(0, 3);
	...
	findDrum(nn: number): number {
		for (var i = 0; i < this.drumKeys().length; i++) {
			let num = this.drumKeys()[i].substring(0, 2);
			if (nn == parseInt(num)) {
				return i;
			}
		}
